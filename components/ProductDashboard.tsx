...
// type Order = {
//   orderid: number;
//   status: string; // "Aceptada" | "Completada" | "Entregada"
//   orderdate: string;
...
//   // Función para finalizar una orden (pasar de "Aceptada" a "Completada")
...
//     try {
//       const response = await fetch(`${API_URL}/orders/complete/${order.orderid}/`, {
//         method: 'POST',
//         headers: {
...
//           prevOrders.map((o) => (o.orderid === order.orderid ? { ...o, status: 'Completada' } : o))
//         );
//       } else {
//         console.error('Failed to finalize the order');
//       }
//     } catch (error) {
//       console.error('Error during the finalize request:', error);
//     }
//   };

//   // Función para marcar una orden como entregada (pasar de "Completada" a "Entregada")
...
//     try {
//       const response = await fetch(`${API_URL}/orders/deliver/${order.orderid}/`, {
//         method: 'POST',
//         headers: {
//           Authorization: `Bearer ${accessToken}`,
//           'Content-Type': 'application/json',
...
//       if (response.ok) {
//         setOrders((prevOrders) =>
//           prevOrders.map((o) => (o.orderid === order.orderid ? { ...o, status: 'Entregada' } : o))
//         );
//       } else {
//         console.error('Failed to deliver the order');
//       }
//     } catch (error) {
//       console.error('Error during the deliver request:', error);
//     }
//   };
...
//   // We now have 7 columns total: Orden, Cliente, Estado, Fecha, Total, Detalles (expand), Acciones (finalizar/entregar)
//   return (
...
//                             <>
//                               <span className="mr-1">Colapsar</span>
...
//                         </button>
//                       </td>
//                       {/* Column for acciones (finalizar/entregar) */}
...
//                           <div
//                             className="hover:bg-green-500 px-3 py-1 rounded text-white "
//                           >
//                             - 
//                           </div>
//                         )}
...
//                     {expandedOrders.has(order.orderid) && (
//                       <tr>
//                         {/* Update colSpan to 7 because we now have 7 columns */}
//                         <td colSpan={7} className="px-6 py-4 bg-[#1E2837]">
            ...
            <table className="min-w-full bg-gray-700 text-white rounded-lg">
              <thead>
                <tr className="bg-gray-600">
                  ...
                  <th className="px-6 py-3 text-left text-sm font-semibold">Total</th>
                  <th className="px-6 py-3 text-left text-sm font-semibold">Detalles</th>
                  {/* Center the "Acciones" header text */}
                ...
                {orders.map((order) => (
                  <React.Fragment key={order.orderid}>
                    <tr className="transition-colors">
                      ...
                      {/* Column for acciones (finalizar/entregar), centered */}
                      <td className="px-6 py-4 flex justify-center items-center space-x-2">
                        ...
                        {order.status === 'Entregada' && (
                          <div className="px-3 py-1 rounded text-white text-center">
                            -
                          </div>
                        )}
                      </td>
