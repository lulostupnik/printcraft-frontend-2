...
//             {
//               product: Number(code), // Convert 'code' to string
//               quantity: Number(quantity), // Convert 'quantity' to string
...
export default function PaymentPage() {
  ...
  // Fetch payment_link and redirect
  useEffect(() => {
    if (isLoggedIn && code && quantity) {
      const fetchPaymentLink = async () => {
        try {
          ...
          if (!response.ok) {
            throw new Error('Failed to fetch payment_link');
          }
          const data = await response.json();
          if (data.payment_link) {
            router.push(data.payment_link);
          } else {
            console.error('No payment_link found in response');
          }
        } catch (error) {
          console.error('Error fetching payment_link:', error);
        }
      };

      fetchPaymentLink();
    }
  }, [isLoggedIn, code, quantity, router]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-900 text-white">
      ...
      <main className="container mx-auto px-4 py-8 flex-grow">
        <section className="relative mb-12 bg-gray-800 flex items-center justify-center min-h-[300px]">
          <div className="w-full max-w-xl text-center space-y-4">
            <h2 className="text-4xl font-bold mb-4">Redirigiendo a tu pago...</h2>
            <h3 className="mb-4">
