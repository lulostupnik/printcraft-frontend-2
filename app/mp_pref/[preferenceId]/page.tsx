...
export default function PaymentPage() {
  ...
  // Extract preferenceId from params
  const { preferenceId: urlPreferenceId } = params;

  useEffect(() => {
    if (Array.isArray(urlPreferenceId)) {
      setPreferenceId(urlPreferenceId[0]);
    } else if (urlPreferenceId) {
  ...
  // Redirect to MercadoPago if preferenceId is available
  useEffect(() => {
    if (preferenceId && isLoggedIn) {
      const redirectUrl = `https://www.mercadopago.com.ar/checkout/v1/redirect?pref_id=${preferenceId}`;
      router.push(redirectUrl);
    }
  }, [preferenceId, isLoggedIn, router]);
